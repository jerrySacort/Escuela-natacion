...
/** Edita una alberca (solo superadmin). */
...
/** Elimina una alberca (solo superadmin). Falla si hay grupos que la usan. */
export const DELETE: APIRoute = async ({ params, locals }) => {
  ...
  if (error) {
    // 23503 = violación de llave foránea (la alberca está asignada a un grupo)
    if (error.code === '23503') {
      return json(
