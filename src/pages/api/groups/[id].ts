...
import { parseSchedules } from './index';
...
export const PATCH: APIRoute = async ({ request, params, locals }) => {
  ...
  // Solo superadmin puede mover grupos de sucursal
  ...
  // Reemplazar horarios si vienen en el form
  if (fd.has('schedules')) {
    ...
    if (schedules.length > 0) {
      const { error: insError } = await supabase
        .from('group_schedules')
        .insert(schedules.map((s) => ({ ...s, group_id: id })));
      if (insError) {
