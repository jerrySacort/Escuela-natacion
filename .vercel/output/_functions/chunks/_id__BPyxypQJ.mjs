...
const PATCH = async ({ request, params, locals }) => {
  ...
  if (fd.has("birth_date")) {
    ...
    if (!v || new Date(v) > /* @__PURE__ */ new Date()) {
  ...
  if (photo instanceof File && photo.size > 0) {
    ...
    const { error: upError } = await supabase.storage.from("student-photos").upload(path, photo, { contentType: photo.type });
    if (upError) {
  ...
  const { data, error } = await supabase.from("students").update(update).eq("id", id).select("id").single();
  if (error || !data) {
